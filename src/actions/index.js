...
export const deleteCard = cardId => {
  return dispatch => {
    axios.delete(`/delete/${cardId}`).then(cards => {
...
export const moveCardRight = cardId => {
  ...
  return dispatch => {
    axios.put(`/move/right/${moveId}`).then(cards => {
      dispatch({ type: LOAD_CARDS, cards: cards.data });
    });
  };
};
...
export const moveCardLeft = cardId => {
  let moveId = parseInt(cardId, 10);
  return dispatch => {
    axios.put(`/move/left/${moveId}`).then(cards => {
      dispatch({ type: LOAD_CARDS, cards: cards.data });
    });
  };
};
...
export const editCard = (cardId, edited) => {
  ...
  return dispatch => {
    axios.put(`/edit/${editId}`, querystring.stringify(edited)).then(cards => {
      dispatch({ type: LOAD_CARDS, cards: cards.data });
    });
  };
};
...
export const addCard = card => {
  return dispatch => {
    axios.post('/post', querystring.stringify(card)).then(cards => {
      dispatch({ type: LOAD_CARDS, cards: cards.data });
    });
  };
};
...
export const addUser = user => {
  return dispatch => {
    axios.post('/login/new', querystring.stringify(user)).then(users => {
